...
import { Contact, Track, ReleasePlan, QuickLink } from "../types.ts";

export const generatePitch = async (contact: Contact, context: string): Promise<string> => {
  // Always create a new instance inside the call to handle potentially late-injected API keys
  ...
  try {
    ...
    return "Ошибка генерации питча. Проверьте API ключ.";
  }
};
